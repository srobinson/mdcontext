...
function hybridSearch(query: string, options: {
  ...
  semanticResults.forEach((r, i) => {
    const rank = i + 1;
    const score = (scores.get(r.sectionId) || 0) +
                  (semanticWeight / (k + rank));
...
const scores = await Promise.all(
  documents.map(doc =>
    reranker(`${query} [SEP] ${doc.content}`)
  )
);
```
...
**Performance**: Engines with integrated filtering maintain recall and often get *faster* with filters (less work to do).
...
```typescript
function search(query: string, options: { quality?: 'fast' | 'balanced' | 'thorough' }) {
  const efSearch = {
    fast: 64,
    balanced: 100,
    thorough: 256
